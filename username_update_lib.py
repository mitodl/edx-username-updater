...
class Updater:
    ...
    def get_previous_result_dict(self):
        ...
        if not result_json_filename and not result_json_value:
            raise ImproperlyConfigured(
                "Need to specify a valid file path (full or partial) for the {} env var, or some valid json for the {} env var.".format(
        ...
        possible_xpro_result_file_paths = [
            join_path(result_json_filename),
            join_path(RESULT_JSON_DIR_PATH, result_json_filename),
            join_path(
                RESULT_JSON_DIR_PATH,
                "{}_{}.json".format(self.settings.input_filename_base, result_json_filename),
            ),
        ]
        ...
        raise ImproperlyConfigured(
            "Could not find an xPro result JSON file at any of these paths: {}".format(
    ...
    def perform_updates_and_handle_results(self):
        ...
        if self.settings.input_filename_env:
            previous_step_update_results = self.get_previous_result_dict()
        now = datetime.now(tz=pytz.UTC)
        result_dict = self.perform_username_updates(
            username_update_list=previous_step_update_results.get("updated", None)
        )
        result_dict["run_date"] = now.isoformat()
        if previous_step_update_results:
